(function () {
  ...
  function bindMobileHomeMenu() { const btn = document.getElementById('mobileMenuBtn'); const menu = document.getElementById('mobileHomeMenu'); if (!btn || !menu || btn.dataset.boundMobileMenu) return; btn.dataset.boundMobileMenu = '1'; btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); toggleMobileHomeMenu(); }); menu.addEventListener('click', (e) => { const link = e.target.closest('a'); const action = e.target.closest('[data-mobile-action]'); if (action) { const type = action.getAttribute('data-mobile-action'); if (type === 'theme') toggleTheme(); if (type === 'search') { const searchBtn = document.getElementById('searchIconBtn'); if (searchBtn) searchBtn.click(); } if (type === 'lang') { const langBtn = document.getElementById('langSwitchBtn'); if (langBtn) langBtn.click(); } return; } if (link) closeMobileHomeMenu(); }); document.addEventListener('click', (e) => { if (!menu.contains(e.target) && !btn.contains(e.target)) closeMobileHomeMenu(); }); document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeMobileHomeMenu(); }); window.addEventListener('resize', () => { if (window.innerWidth > 640) closeMobileHomeMenu(); }); window.addEventListener('DOMContentLoaded', () => { const menuBtn = document.getElementById('mobileMenuBtn'); if (menuBtn) menuBtn.style.display = 'inline-flex'; }); }
