(function () {
  ...
  function containsForbiddenContent(text) {
    ...
    if (session && isApprovedEditor(session)) return false;
    var patterns = [ /magnet\s*:\s*\?/i, /magnet%3a%3f/i, /赌博|博彩|下注|彩票|代开/i, /色情|裸聊|成人视频|自拍偷拍/i, /毒品|吸毒|贩毒|冰毒|海洛因|大麻/i, /血腥|屠杀|碎尸|爆头|斩首|虐杀/i, /习近平|中国共产党|反党|推翻政权|台独|港独|疆独/i ];
  ...
  function buildCopyableUrl(url) {
    var safeUrl = String(url || '').trim();
    if (!safeUrl) return '';
    var href = /^https?:\/\//i.test(safeUrl) ? safeUrl : 'https://' + safeUrl;
    return '<span class="article-link-copy"><a href="' + sanitizeText(href) + '" target="_blank" rel="noopener noreferrer">' + sanitizeText(safeUrl) + '</a><button type="button" class="article-link-copy__btn" data-copy-link="' + sanitizeText(href) + '">一键复制</button></span>';
  }

  function enhanceUrlsInHtml(html) {
    var input = String(html || '');
    if (!input) return '';
    var host = document.createElement('div');
    host.innerHTML = sanitizeHtmlRich(input);
    host.innerHTML = host.innerHTML.replace(/(https?:\/\/[^\s<]+|www\.[^\s<]+)/gi, function (m) { return buildCopyableUrl(m); });
    return host.innerHTML;
  }

  function canAdminBypass(session) {
    return isApprovedEditor(session);
  }
  ...
  function isSensitiveText(text) {
    ...
    var session = window.TengyouSession.get();
    if (session && isAdminEmail(session.email)) return false;
    var compact = value.replace(/\s+/g, '');
    var patterns = [
      /(?:https?:\/\/|www\.)/i,
      /[\w.-]+\.(?:com|net|org|cn|cc|top|xyz|icu|me|tv|info|app)(?:[\/?#][^\s]*)?/i,
      /magnet\s*:\s*\?/i,
      /magnet%3a%3f/i,
      /赌博|博彩|下注|彩票|赌钱|赌局/i,
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var videoBtn = document.getElementById('addVideoBtn');
    var videoInput = document.getElementById('homePostVideosInput');
    var videoList = document.getElementById('homePostVideosList');
    var selectedVideos = [];
    ...
    function syncVideoList() { if (!videoList) return; videoList.innerHTML = ''; selectedVideos.forEach(function (src, idx) { var item = document.createElement('div'); item.className = 'home-post-image-item'; item.innerHTML = '<video controls src="' + src + '" style="width:100%;max-width:180px;border-radius:10px;background:#000"></video><div class="home-post-image-item__foot"><p class="home-post-image-item__label">' + (idx === 0 ? '主视频' : '视频 ' + (idx + 1)) + '</p><button type="button" class="home-post-image-item__remove" data-remove-home-video="' + idx + '">删除</button></div>'; videoList.appendChild(item); }); }
    ...
    if (videoBtn && videoInput) { videoBtn.addEventListener('click', function () { var session = window.TengyouSession.get(); if (!session || session.email !== APPROVED_EMAIL) return; videoInput.click(); }); videoInput.addEventListener('change', function () { Array.prototype.slice.call(videoInput.files || []).forEach(function (file) { var reader = new FileReader(); reader.onload = function () { selectedVideos.push(String(reader.result || '')); syncVideoList(); }; reader.readAsDataURL(file); }); videoInput.value = ''; }); }
    ...
    if (videoList) videoList.addEventListener('click', function (e) { var removeVideoBtn = e.target.closest('[data-remove-home-video]'); if (!removeVideoBtn) return; var idx2 = parseInt(removeVideoBtn.getAttribute('data-remove-home-video') || '0', 10); if (isNaN(idx2)) return; selectedVideos.splice(idx2, 1); syncVideoList(); });
    if (postForm) postForm.addEventListener('submit', function (e) { e.preventDefault(); var session = window.TengyouSession.get(); if (!session || session.email !== APPROVED_EMAIL) return; syncBodyInput(); var title = (postForm.querySelector('input[name="title"]') || {}).value || ''; var bodyHtml = (postBodyInput ? postBodyInput.value : '') || ''; title = title.trim(); bodyHtml = normalizeEditorHtml(bodyHtml).trim(); if (!title || !bodyHtml || !selectedImages.length) { window.alert('请填写标题、正文并至少添加一张图片。'); return; } if (isSensitiveText(title) || isSensitiveText(bodyHtml)) { window.alert('内容包含违规信息，已自动阻止发布。'); return; } var plainText = (postBodyEditor ? postBodyEditor.innerText : bodyHtml).trim(); var articles = getArticles(); articles.push({ id: Date.now(), title: title, image: selectedImages[0], images: selectedImages.slice(), videos: selectedVideos.slice(), body: plainText, bodyHtml: bodyHtml, ts: Date.now(), authorEmail: session.email, likes: 0, comments: [] }); saveArticles(articles); selectedImages = []; selectedVideos = []; syncImageList(); syncVideoList(); if (postBodyEditor) postBodyEditor.innerHTML = ''; if (postBodyInput) postBodyInput.value = ''; postForm.reset(); closePostModal(); if (homeList) homeList.setAttribute('data-current-page', '1'); renderHomeArticles(); renderHotRankHome(); });
    ...
    if (homeList) homeList.addEventListener('click', function (e) {
      ...
      if (manageType === 'delete') {
        ...
        var submitHandler = function (evt) {
          ...
          if (isSensitiveText(newTitle) || isSensitiveText(newBodyHtml)) { editSetStatus('保存失败：内容包含违规信息。', 'error'); return; }
    ...
    if (boardView) {
      ...
      if (boardForm) boardForm.addEventListener('submit', function (e) { e.preventDefault(); var session = window.TengyouSession.get(); if (!session) return; var ta = boardForm.querySelector('textarea[name="content"]'); var body = ta ? ta.value.trim() : ''; if (!body) return; if (isSensitiveText(body)) { window.alert('内容包含违规信息，已自动阻止发布。'); return; } var board = getBoard(); board.posts.push({ id: Date.now(), nickname: session.username, body: body, ts: Date.now(), comments: [] }); saveBoard(board); boardForm.reset(); renderBoard(); });
