(function () {
  ...
  function applyI18n(lang) { const dict = I18N[lang] || I18N['zh-CN']; document.querySelectorAll('[data-i18n]').forEach((el) => { const key = el.getAttribute('data-i18n'); if (dict[key]) el.textContent = dict[key]; }); document.querySelectorAll('[data-i18n-placeholder]').forEach((el) => { const key = el.getAttribute('data-i18n-placeholder'); if (dict[key]) el.setAttribute('placeholder', dict[key]); }); const langBtn = document.getElementById('langSwitchBtn'); const langIcon = document.querySelector('.lang-switch__icon'); if (langBtn) { langBtn.setAttribute('aria-label', '切换语言'); langBtn.setAttribute('title', '切换语言'); if (langIcon) langIcon.textContent = '文/A'; } const mobileMenu = document.getElementById('mobileHomeMenu'); if (mobileMenu) { const langMenuBtn = mobileMenu.querySelector('[data-mobile-action="lang"] span:last-child'); if (langMenuBtn) langMenuBtn.textContent = '文/A'; const themeBtnText = mobileMenu.querySelector('[data-mobile-action="theme"] span:last-child'); if (themeBtnText) themeBtnText.textContent = '☀️/🌙 白天/黑夜'; const searchBtnText = mobileMenu.querySelector('[data-mobile-action="search"] span:last-child'); if (searchBtnText) searchBtnText.textContent = '🔎搜索'; } if (window.initHitokotoCarousel) window.initHitokotoCarousel(); }
  ...
  const MOBILE_LANGS = ['zh-CN', 'zh-TW', 'en', 'ko', 'ja', 'fr', 'it', 'de', 'ru'];
  ...
  function openMobileHomeMenu() { const menu = document.getElementById('mobileHomeMenu'); const btn = document.getElementById('mobileMenuBtn'); if (!menu || !btn) return; document.body.classList.add('mobile-home-menu-open'); menu.hidden = false; menu.classList.add('is-open'); btn.setAttribute('aria-expanded', 'true'); }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try { bindMobileHomeMenu(); } catch (err) { console.warn('[bindMobileHomeMenu]', err); }
