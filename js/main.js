(function () {
  ...
  async function loadState() { const res = await apiFetch('/api/state'); stateCache.data = res.item || stateCache.data; return stateCache.data; }
  ...
  async function apiLoadArticles() { try { const res = await apiFetch('/api/articles'); const apiItems = Array.isArray(res.items) ? res.items : []; const localItems = loadLocalArticles(); const merged = [...apiItems]; localItems.forEach((item) => { if (!merged.some((x) => String(x.id) === String(item.id))) merged.push(item); }); saveLocalArticles(merged); return merged; } catch (e) { const localItems = loadLocalArticles(); if (localItems.length) return localItems; return Array.isArray(stateCache.data.articles) ? stateCache.data.articles : []; } }
  ...
  function getArticlesStorageKey() { const session = TengyouSession.get(); return 'tengyou-home-articles:' + String(session && session.email ? session.email : 'guest').toLowerCase(); }
  ...
  function getArticleStorageKey() { return 'tengyou-home-articles'; }
  function saveLocalArticles(items) { try { localStorage.setItem(getArticleStorageKey(), JSON.stringify(Array.isArray(items) ? items : [])); } catch (e) {} }
  function loadLocalArticles() { try { const raw = localStorage.getItem(getArticleStorageKey()); return raw ? JSON.parse(raw) : []; } catch (e) { return []; } }
