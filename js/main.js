(function () {
  ...
  async function renderHome() {
    ...
    const rank = document.getElementById('hotRankHome'); if (rank) { rank.innerHTML = ''; const rankedItems = items.slice().sort((a, b) => articleStats(b).heat - articleStats(a).heat || (b.ts || 0) - (a.ts || 0)); rankedItems.slice(0, 10).forEach((a, idx) => { const row = document.createElement('a'); row.className = 'hot-rank__title-item'; row.href = 'ranking.html#article-' + encodeURIComponent(a.id); row.innerHTML = '<span class="hot-rank__num">' + (idx + 1) + '</span><span class="hot-rank__title">' + esc(a.title) + '</span><span class="hot-rank__heat">' + articleStats(a).heat + '</span>'; rank.appendChild(row); }); }
  ...
  function initAuth() {
    ...
    if (loginForm && !loginForm.dataset.boundAuth) {
      ...
      loginForm.addEventListener('submit', async (e) => {
        ...
        let loginOk = false;
        try { const res = await apiLogin({ account, password }); const item = res.item || {}; if (!item || !item.username || !item.email) throw new Error('登录失败'); TengyouSession.set(item.username || account, item.email || email); loginOk = true; } catch (err) { console.warn('[login-error]', err); TengyouSession.clear(); alert((err && err.message) ? err.message : '登录失败，请检查账号、密码或后端服务'); return; }
